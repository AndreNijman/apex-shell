...
ShellRoot {
    ...
    property Process _sh: Process {
        ...
        onExited: function (code) { root._shSettle.restart() }
    }
    property Timer _shSettle: Timer {
        ...
        onTriggered: {
            const cb = root._shCb
            root._shCb = null
            try { if (cb) cb(root._shOut) }
            catch (e) { root.check("callback threw", false, String(e)); root.next() }
        }
    }
    function sh(cmd, cb) {
        root._shOut = ""
    ...
    function next() {
        ...
        root._step++
        // A step that throws must report a failed assertion and carry on. This
        // suite is meant to be run against the UNFIXED tree to show it goes
        // red, and there every call is to a property that does not exist yet:
        // without this the first exception stops the chain and the only output
        // is the watchdog's "did not finish", which names nothing.
        try {
            fn()
        } catch (e) {
            root.check("step " + root._step + " threw", false, String(e))
            root.next()
        }
    }
    ...
    property Timer _settle: Timer {
        ...
        onTriggered: {
            tries++
            // Defensive for the same reason: on a tree without these
            // properties the expression must be false, not an exception
            // thrown out of a repeating timer.
    ...
    Component.onCompleted: {
        root.steps = [
            ...
            // Everything above exercises the service. A bad binding in the PAGE
            // takes down the whole shell — one stray conflict marker in an
            // unrelated file was enough to stop it loading while this branch was
            // being written — and no assertion here would have noticed.
            function () {
                const c = Qt.createComponent(
                    Qt.resolvedUrl("./src/services/config_tab/pages/InputPage.qml"))
                root.check("the Input page compiles",
                           c.status === Component.Ready, c.errorString())
                if (c.status === Component.Ready) {
                    const obj = c.createObject(null)
                    root.check("the Input page instantiates", obj !== null)
                    if (obj) obj.destroy()
                }
                root.next()
            },
