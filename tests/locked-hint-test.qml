...
ShellRoot {
    ...
    // Phase 3 — a lock/unlock/lock flicker coalesces.
    function phase3() {
        ...
        settle(function () {
            pause(400, function () {
                readLog(function () {
                    const got = rootScope.fresh()
                    rootScope.check("three flips while a call is in flight make one chain, not three",
                                    got.length === 3)
                    if (got.length >= 3)
                        rootScope.check("and the one that lands is the last value asked for",
                                        isSetHint(got[got.length - 1]) && got[got.length - 1].indexOf("b false") >= 0)
                    rootScope.keep()
                    phase4()
                })
            })
