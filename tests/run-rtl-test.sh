...
for d in $("${qtool%% *}" -query QT_INSTALL_QML 2>/dev/null) \
    ...
    [ -n "$d" ] && [ -e "$d/Quickshell/_Window/quickshell-window.qmltypes" ] \
        && { qsqml="$d/Quickshell/_Window/quickshell-window.qmltypes"; break; }
done
if [ -z "$qsqml" ]; then
    ...
    chain="$(python3 - "$qsqml" <<'CHAINPY'
import re, sys
proto = {}
for blk in re.split(r'\n(?=    Component \{)', open(sys.argv[1], encoding="utf-8", errors="replace").read()):
