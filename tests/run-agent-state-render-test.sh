#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
#  Run tests/agent-state-render-test.qml — the Agent Center's state colours, as
#  Qt resolves them rather than as the source reads.
#
...
#  display at all.
#
#  The QML opens no window of its own either. Belt and braces, because the cost
#  of being wrong here is somebody's workspace.
#
...
# Say what is wrong rather than letting QML report it as a missing type. On the
# tree before P0-021 there is no badge at all: every row decides a state's
# colour inline, and four of the seven states resolve to the palette's own
# foreground. "StateBadge is not a type" is a true but useless way to say that.
if [[ ! -f "$root/src/services/agents/StateBadge.qml" ]]; then
    echo "FAIL: this tree has no StateBadge — agent states are coloured inline"
    echo "      by each row, which is the defect this test measures. Nothing to"
    echo "      render."
    exit 1
fi
grep -q "^StateBadge " "$root/src/services/qmldir" || {
    # AgentCenter is loaded THROUGH src/services/qmldir, so its directory is not
    # on the import path and a sibling that is not registered there is invisible
    # to it — with the shell failing to load at all, not the page. The qmldir
    # already warns about this for the rows; the badge is one more.
    echo "FAIL: StateBadge is not registered in src/services/qmldir, so the"
    echo "      Agent Center cannot see it and the whole shell fails to load."
    exit 1; }
...
W="$(mktemp -d)"
staged="$root/.agent-state-render-test.qml"
comp_pid=""
cleanup() {
...
cp "$here/agent-state-render-test.qml" "$staged"
...
echo "$out" | grep -E "PASS|FAIL|background=|colour=|passed=" || true
...
if ! echo "$out" | grep -q "agent-state-render: passed="; then
    echo "$out" | tail -25
...
echo "RESULT: every agent state resolved a distinct, readable colour in both palettes"
