...
ShellRoot {
    ...
    function openPage(then) {
        ...
        // assertion, because the interval is fifteen seconds and the log is
        ...
        root.readLog(function () {
            ...
            root.check("and its sweep timer is not running",
                       LidService._tick.running === false)
            root.page.onScreen = true
            root.check("showing the page takes a reference", LidService.refCount > 0)
            root.check("and starts the sweep timer", LidService._tick.running === true)
    ...
    /// The other half of "no process runs while nobody is looking": handing the
    /// reference back stops it. Last in the phase, because the page is left
    /// off-screen afterwards.
    function assertPollerStops() {
        root.page.onScreen = false
        root.eq("hiding the page hands the reference back", LidService.refCount, 0)
        root.check("and stops the sweep timer", LidService._tick.running === false)
        root.readLog(function () {
