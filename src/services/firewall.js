...
function parseStatusJson(exitCode, stdout) {
    ...
    //
    // The first draft of this function read `!== null && !== undefined`, which
    // is the shortcut that makes them identical. What actually refuses a
    // missing key is the `!== null` test below sending `undefined` into the
    // array branch; this line is here so that re-adding the shortcut is a
    // deliberate act rather than a tidy-up, and the case that names it goes
    // red when the shortcut comes back.
