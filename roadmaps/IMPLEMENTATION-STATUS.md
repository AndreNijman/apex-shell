...
| 15 Top bar redesign | IN PROGRESS (15a) | 15a: focus-mode collapse (height, exclusive zone, the two opacities) on the page beat instead of animDuration/InOutCubic; the left notch's width animates like the centre's (it snapped); the C.6/D.7 hairline — `geometry.js barHairline`, an open inset path drawn as a second ShapePath, stopping short of the right notch while a pane hangs from it; notch padding 14, left min 140, right min 160 (brief §C.1/§D.2); the open pill on the left notch's control. 15b: icons and colour rules | 2, 3, 5 | geometry 165/0 (hairline inside the fill, no kinks, stops before an attached notch); seam pixel check under an open panel = the bar's own colour, not the hairline; labwc matrix, scaling (notchPadding expectations moved to 14), rtl, popup smoke | notchRadius default 15 → 12 stays a product call |
...
- Committed through 15a. Next: 15b (IconBtn on ApexIconButton, the bar's
  icon size/font/colour rules, spacing, the clock), then 16.
